...
# We use 'set -e' to exit on unexpected errors, but will disable it for tests
# where a non-zero exit code is the expected outcome.
set -e

info "--- Phase 1: Linting and Formatting ---"
FMT_OUTPUT=$(go fmt ./...)
if [ -n "$FMT_OUTPUT" ]; then
    error "Go files are not formatted. Please run 'go fmt ./...' and commit the changes.\nFiles changed:\n$FMT_OUTPUT"
fi
go vet ./...
...
set +e
output_clean=$(docker run --rm -v "$REPO_CLEAN_PATH:/scan" "$DOCKER_IMAGE_NAME" git /scan 2>&1)
exit_code_clean=$?
set -e
...
if ! echo "$output_clean" | grep -q "No secrets found"; then
    error "Expected 'No secrets found' for clean repo, but it was not in the output. Output:\n$output_clean"
fi
...
set +e
output_secret=$(docker run --rm -v "$REPO_SECRET_PATH:/scan" "$DOCKER_IMAGE_NAME" git /scan 2>&1)
exit_code_secret=$?
set -e
...
if [ -z "$GEMINI_API_KEY" ]; then
    ...
    set +e
    output_secret_ai=$(docker run --rm -v "$REPO_SECRET_PATH:/scan" -e GEMINI_API_KEY="$GEMINI_API_KEY" "$DOCKER_IMAGE_NAME" git /scan 2>&1)
    exit_code_secret_ai=$?
    set -e
    ...
    if ! echo "$output_secret_ai" | grep -q "github-pat"; then
        error "Expected finding for 'github-pat' was not found in GenAI mode. Output:\n$output_secret_ai"
    fi
    ...
    set +e
    output_fp_ai=$(docker run --rm -v "$REPO_FP_PATH:/scan" -e GEMINI_API_KEY="$GEMINI_API_KEY" "$DOCKER_IMAGE_NAME" git /scan 2>&1)
    exit_code_fp_ai=$?
    set -e
    ...
    if ! echo "$output_fp_ai" | grep -q "No secrets found"; then
        error "Expected 'No secrets found' for false positive repo, but it was not in the output. Output:\n$output_fp_ai"
    fi
